...
<body>
	...
	<table id="dg" title="管理员列表" style="width:100%;height:500px"
			data-options="rownumbers:true,singleSelect:true,pagination:true,url:'./adminServlet?method=getAdminJson',method:'get'">
		<thead>
			<tr>
				<th data-options="field:'admin_id',width:80">管理员ID</th>
				<th data-options="field:'admin_name',width:100">管理员名字</th>
			</tr>
		</thead>
	</table>
		...
		$(function(){
			...
			pager.pagination({
				buttons:[{
					...
					handler:function(){
						$("#password-panel").hide();
						$("#id").textbox('setValue',$('#dg').datagrid('getSelected').admin_id);//赋值
						$("#name").textbox('setValue',$('#dg').datagrid('getSelected').admin_name);//赋值
					...
					iconCls:'icon-remove',
					handler:function(){
						var admin_id = $('#dg').datagrid('getSelected').admin_id;//赋值
						$.messager.confirm('警告', '确定删除该账号吗', function(r){
							if (r){
								$.post('./adminServlet?method=deleteAdmin',{admin_id : admin_id}, function(data) {
