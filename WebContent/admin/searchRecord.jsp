...
<body>
	<table id="dg" class="easyui-datagrid" title="选课课程列表"
			...
			data-options="
				singleSelect: true,
				fitColumns: true,
				url:'adminServlet?method=getAllCourse'
				
			">
			<thead>
				<tr>
					<th width="120" sortable="true"
						data-options="field:'plan_id',align:'center'">
						课程计划ID
					</th>
					<th width="120" sortable="true"
		...
		<div id="studentListWD" class="easyui-window" title="选课学生信息" data-options="modal:true,collapsible:false,closed:true,maximizable:false,minimizable:false,resizable:false" style="width:850px;height:400px">
			...
			data-options="
				...
				toolbar: '#tb',
				fitColumns: true,
				url:'../teacher/teacherServlet?method=getMyStudent'
			">
			<thead>
				<tr>
					<th width="120"
		...
		<div id="tb" style="height:auto">
			...
			<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true" onclick="deleteall()">清除记录</a>
		</div>
		</div>
		<div id="dialog2"></div> 
		...
		function deleteall() {
			$.messager.confirm('警告', '你确定要清楚该课程的选课记录吗<br />', function(r){
				if (r){
					$.post("./adminServlet?method=deleteRecord",{course_id : $('#dg').datagrid('getSelected').course_id},function(data){
