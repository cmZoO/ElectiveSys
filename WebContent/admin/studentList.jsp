...
<body>
	...
	<table id="dg" title="学生列表" style="width:100%;height:500px"
			data-options="rownumbers:true,singleSelect:true,pagination:true,url:'./adminServlet?method=getStudentJson',method:'get'">
		<thead>
			<tr>
				<th data-options="field:'stu_id',width:80">学号</th>
				<th data-options="field:'stu_name',width:100">学生名字</th>
				<th data-options="field:'register_date',width:100">入学时间</th>
			</tr>
		</thead>
	</table>
	...
	<div id="userWindow" class="easyui-window" title="修改资料" data-options="modal:true,collapsible:false,closed:true,maximizable:false,minimizable:false,resizable:false" style="width:550px;height:400px;padding:10px;">
			<div style="width: 400px; padding: 30px 60px;">
				<form id="ff" method="post"
					action="./adminServlet?method=studentChange">
					...
					<div style="margin-bottom: 20px">
						<input class="easyui-datebox theme-textbox-radius " id="date"
							name="date" style="width: 100%"
							data-options="required:true, label:'入学时间:'" />
					</div>
		...
		$(function(){
			...
			pager.pagination({
				buttons:[{
					...
					handler:function(){
						$("#password-panel").hide();
						$("#id").textbox('setValue',$('#dg').datagrid('getSelected').stu_id);//赋值
						$("#name").textbox('setValue',$('#dg').datagrid('getSelected').stu_name);//赋值
						$("#date").textbox('setValue',$('#dg').datagrid('getSelected').register_date);//赋值
